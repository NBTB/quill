package scripts
{
    ...
    public class HuntImporter extends EventDispatcher
    {
				...
				for each(var formatParams in formats)
				{
					...
					for each(var attrib in attribs)
					{
						if(attrib.name() == "name")
						{
                            ...
                            else if(formatName == "alt_intro")
                                BaseMenu.altIntroFormat = newFormat;
		...
		private function parseMenuContent(menuContent:XML)
		{
            ...
            if(menuContent.hasOwnProperty("welcome_subtitle")) {
                IntroMenu.introSubTitle = menuContent.welcome_subtitle;
            }
