package scripts
{
	...
	public class BaseMenu extends MovieClip
	{
        ...
        protected var overrideBorderEnabled:Boolean = false;
        ...
        public static var altIntroFormat:TextFormat = null;					//alternate text format for intro menu text
		...
		public function BaseMenu(xPos:int, yPos:int, widthVal:int, heightVal:int, closeable:Boolean = true, scrollable:Boolean = true, draggable:Boolean = true, opacity:Number = DEFAULT_OPACITY, overrideBorderEnabled:Boolean = false):void
		{
            ...
            //Set overriding border enabled (force border on)
            this.overrideBorderEnabled = overrideBorderEnabled;
			...
			if(closeable)
			{
				...
				var closeButtonRect:Rectangle = new Rectangle(widthVal - 19, 1, 19, 19);
			...
			if(draggable)
			{
				//create drag cap
				dragCap = new Sprite();
				dragCap.graphics.lineStyle(1, 0x464f63);
		...
		public function createBackground(widthVal:int, heightVal:int, opacity:Number = DEFAULT_OPACITY):void
		{
			...
			if(menuBorderEnabled || this.overrideBorderEnabled) {
